...
class MoviesForm extends React.Component {
  ...
  handleSave = () => {
    ...
    id ?
      updateMovie({
        id, name, genre, rate: Number(rate),
        directorId, watched: Boolean(watched),
      }) :
      addMovie({
        name, genre, rate: Number(rate),
        directorId, watched: Boolean(watched),
      });
    onClose();
  };
  ...
  render() {
    const {
      classes, open, handleChange, handleSelectChange,
      handleCheckboxChange, selectedValue = {}, data = {},
    } = this.props;
